...
// Stack implementation using the structure for implementing LIFO approach in C 
typedef struct Lifo {
...
int CLifo_setSize(SLifo *p, int nSize){
  ...
  p->pLifo = realloc(p->pLifo, nSize * sizeof(int));
  if(!p->pLifo) return 2; // error allocating memory 
  p->nSize = nSize;
  return 0;
}
...
int CLifo_push(SLifo *p, int n){
  if(p->iTop == p->nSize-1){ // if the stack is full
    CLifo_setSize(p, p->nSize+1); // extend the size
  }
